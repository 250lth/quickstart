...
while true; do
  ENV_STATE=$(docker run \
    ...
    $curlprefix/curl \
      ...
      "https://$rancher_server_ip/v3/clusterregistrationtoken?name=my-sandbox" | jq -r '.data[].nodeCommand')
...
CLUSTERRESPONSE=$(docker run --net host \
    --rm \
    $curlprefix/curl -s "https://$rancher_server_ip/v3/clusters?name=my-sandbox" -H 'content-type: application/json' -H "Authorization: Bearer $LOGINTOKEN" --insecure)
